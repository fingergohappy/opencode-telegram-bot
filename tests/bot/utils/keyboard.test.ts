...
describe("bot/utils/keyboard", () => {
  it("creates main keyboard with defaults", () => {
    ...
    expect(getButtonText(keyboard.keyboard[0][1])).toBe("🤖 openrouter\nopenai/gpt-4o");
    expect(getButtonText(keyboard.keyboard[0][2])).toBe("💡 Default");
    expect(keyboard.resize_keyboard).toBe(true);
    expect(keyboard.is_persistent).toBe(true);
  });

  it("creates main keyboard with custom variant", () => {
    ...
    expect(getButtonText(keyboard.keyboard[0][1])).toBe("🤖 provider\nmodel");
    expect(getButtonText(keyboard.keyboard[0][2])).toBe("⚡ Fast");
  });
